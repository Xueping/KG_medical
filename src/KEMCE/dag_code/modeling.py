...
class BertAttentionDag(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, input_tensor_dag):
        self_output = self.self(input_tensor, attention_mask)
        self_output_dag = self.self_dag(input_tensor_dag, attention_mask)

        attention_output = self.output(self_output, input_tensor)
        attention_output_dag = self.output_dag(self_output_dag, input_tensor_dag)
        return attention_output, attention_output_dag
...
class BertLayerDag(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, hidden_states_dag):
        attention_output, attention_output_dag = \
            self.attention(hidden_states, attention_mask, hidden_states_dag)
        intermediate_output = self.intermediate(attention_output, attention_output_dag)
        layer_output, layer_output_dag = \
            self.output(intermediate_output, attention_output, attention_output_dag)
        return layer_output, layer_output_dag
