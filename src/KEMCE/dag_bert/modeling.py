...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        config_ent.hidden_size = config.hidden_size
        config_ent.num_attention_heads = 4

        self.self_ent = BertSelfAttention(config_ent)
        self.output_ent = BertSelfOutput(config_ent)

    def forward(self, input_tensor, input_tensor_ent, attention_mask, attention_ent_mask):
...
class BertLayerMix(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_ent, attention_mask, extended_ent_mask, ent_mask):
        ...
        layer_output, layer_output_ent = \
            self.output(intermediate_output, attention_output, attention_output_ent)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_ent, attention_mask, extended_ent_mask, ent_mask):
        attention_output, attention_output_ent = \
            self.attention(hidden_states, hidden_states_ent, attention_mask, extended_ent_mask)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_ent, attention_mask,
                extended_ent_mask, ent_mask, output_all_encoded_layers=True):
        all_encoder_layers = []

        for layer_module in self.layer:
            hidden_states, hidden_states_ent = \
                layer_module(hidden_states, hidden_states_ent, attention_mask,
...
class BertForMaskedLM(PreTrainedBertModel):
    ...
    def forward(self, input_ids, input_ents,
                attention_mask=None, token_type_ids=None, masked_lm_labels=None):
        sequence_output, _ = self.bert(input_ids, input_ents, attention_mask, token_type_ids,
                                       output_all_encoded_layers=False)
