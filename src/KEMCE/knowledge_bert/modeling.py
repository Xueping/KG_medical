...
class TextCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(batch_size * step_size,x.shape[2], x.shape[3])
        ...
        logit = logit.reshape(batch_size, step_size, logit.shape[1])
...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        config_desc = copy.deepcopy(config)
        config_desc.hidden_size = 100
        config_desc.num_attention_heads = 4

        self.self_desc = BertSelfAttention(config_desc)
        self.output_desc = BertSelfOutput(config_desc)

    def forward(self, input_tensor, input_tensor_ent, input_tensor_desc, attention_mask, attention_ent_mask):
        self_output = self.self(input_tensor, attention_mask)
        self_output_ent = self.self_ent(input_tensor_ent, attention_ent_mask)
        self_output_desc = self.self_desc(input_tensor_desc, attention_ent_mask)

        attention_output = self.output(self_output, input_tensor)
        attention_output_ent = self.output_ent(self_output_ent, input_tensor_ent)
        attention_output_desc = self.output_desc(self_output_desc, input_tensor_desc)
        return attention_output, attention_output_ent, attention_output_desc
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense_desc = nn.Linear(100, config.intermediate_size)

        self.intermediate_act_fn = ACT2FN[config.hidden_act] \
            if isinstance(config.hidden_act, str) else config.hidden_act

    def forward(self, hidden_states, hidden_states_ent, hidden_states_desc):
        hidden_states_ = self.dense(hidden_states)
        # print('dtype of hidden_states_ent: ', hidden_states_ent.dtype)
        hidden_states_desc_ = self.dense_desc(hidden_states_desc)
        hidden_states_ent_ = self.dense_ent(hidden_states_ent.float())
        hidden_states = self.intermediate_act_fn(hidden_states_+hidden_states_ent_+hidden_states_desc_)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense_desc = nn.Linear(config.intermediate_size, 100)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.LayerNorm_ent = BertLayerNorm(100, eps=1e-12)
        self.LayerNorm_desc= BertLayerNorm(100, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, hidden_states_, input_tensor, input_tensor_ent, input_tensor_desc):
        ...
        hidden_states_desc = self.dense_desc(hidden_states_)
        hidden_states_desc = self.dropout(hidden_states_desc)
        hidden_states_desc = self.LayerNorm_desc(hidden_states_desc + input_tensor_desc)

        return hidden_states, hidden_states_ent, hidden_states_desc
...
class BertLayerMix(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_ent, hidden_states_desc, attention_mask, extended_ent_mask, ent_mask):
        attention_output = self.attention(hidden_states, attention_mask)
        # print(hidden_states_ent.shape, ent_mask.shape)
        attention_output_ent = hidden_states_ent * ent_mask
        attention_output_desc = hidden_states_desc * ent_mask
        intermediate_output = self.intermediate(attention_output, attention_output_ent, attention_output_desc)
        layer_output, layer_output_ent, layer_output_desc = \
            self.output(intermediate_output, attention_output, attention_output_ent, attention_output_desc)

        return layer_output, layer_output_ent, layer_output_desc
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_ent, hidden_states_desc, attention_mask, extended_ent_mask, ent_mask):
        attention_output, attention_output_ent, attention_output_desc = \
            self.attention(hidden_states, hidden_states_ent, hidden_states_desc, attention_mask, extended_ent_mask)
        attention_output_ent = attention_output_ent * ent_mask
        attention_output_desc = attention_output_desc * ent_mask
        intermediate_output = self.intermediate(attention_output, attention_output_ent, attention_output_desc)
        layer_output, layer_output_ent, layer_output_desc = \
            self.output(intermediate_output, attention_output, attention_output_ent, attention_output_desc)
        return layer_output, layer_output_ent, layer_output_desc
...
class BertLayer_simple(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_ent, hidden_states_desc, attention_mask, extended_ent_mask, ent_mask):
        ...
        return layer_output, hidden_states_ent, hidden_states_desc
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_ent, hidden_states_desc, attention_mask,
                extended_ent_mask, ent_mask, output_all_encoded_layers=True):
        all_encoder_layers = []

        for layer_module in self.layer:
            hidden_states, hidden_states_ent, hidden_states_desc = \
                layer_module(hidden_states, hidden_states_ent,
                             hidden_states_desc, attention_mask,
                             extended_ent_mask, ent_mask)

            # print('embedding shape of hidden_states: ', hidden_states.shape)
            # print('embedding shape of hidden_states_ent: ', hidden_states_ent.shape)
            # print('embedding shape of hidden_states_desc: ', hidden_states_desc.shape)
...
class BertModel(PreTrainedBertModel):
    ...
    Outputs: Tuple of (encoded_layers, pooled_output)
        ...
        `pooled_output`: a torch.FloatTensor of size [batch_size, hidden_size] which is the output of a
            ...
            input (`CLF`) to utils on the Next-Sentence task (see BERT's paper).
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, input_desc=None,
                ent_mask=None, output_all_encoded_layers=True):
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, 
        ...
        sequence_output, pooled_output = self.bert(input_ids, attention_mask, input_ent, ent_mask, token_type_ids,
...
class BertForMaskedLM(PreTrainedBertModel):
    ...
    def forward(self, input_ids, input_ents, input_desc,
                attention_mask=None, token_type_ids=None, masked_lm_labels=None):
        sequence_output, _ = self.bert(input_ids, input_ents, input_desc, attention_mask, token_type_ids,
                                       output_all_encoded_layers=False)
