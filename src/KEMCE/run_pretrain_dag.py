...
import torch
from KEMCE.dag_bert import SeqsTokenizer, EntityTokenizer, KemceForPreTraining, BertAdam, BertConfig
from GRAM.gram_helpers import build_tree_with_padding, build_tree
...
def main():
    ...
    #
    # config.dag_leaves_list = torch.tensor(leaves_list).long()
    # config.dag_ancestors_list = torch.tensor(ancestors_list).long()
    # config.dag_mask_list = torch.tensor(mask_list).float()
    ...
    if args.do_train:
        ...
        dataset = BERTDataset(pair_seqs, pair_seqs_mask, pair_seqs_mask_label, seq_tokenizer, ent_tokenize)
    ...
    if args.pretrained_dir is not None:
        ...
        model, _ = KemceForPreTraining.from_pretrained(args.pretrained_dir, config, state_dict)
    else:
