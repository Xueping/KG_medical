...
def collate_rd(batch, num_dx_classes):
    ...
    readmission_label_ls = [x[2][0] for x in batch]
    ...
    output = {"input": torch.tensor(input).long(),
              "input_ent": torch.tensor(ent_input).long(),
              # "input_desc": torch.tensor(desc_input).long(),
...
class FTDataset(Dataset):
    def __init__(self, inputs, labels, seqs_tokenizer, ent_tokenize):
        self.inputs = inputs
        self.labels = labels
    ...
    def __getitem__(self, item):
        ...
        return input_visit, input_ent, self.labels[item]
