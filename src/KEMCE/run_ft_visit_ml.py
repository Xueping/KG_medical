...
from KEMCE.knowledge_bert import SeqsTokenizer, EntityTokenizer, DescTokenizer, \
    KemceFTPrediction, BertAdam, BertConfig
from KEMCE.dataset import FTDataset, collate_ml
...
def main():
    ...
    # training data files
    seqs_file = data_path + 'outputs/kemce/data/mortality_los/mimic.visits'
    labels_file = data_path + 'outputs/kemce/data/mortality_los/mimic.labels'
    ...
    if args.do_train:
        ...
        train_data_loader = DataLoader(dataset, batch_size=args.train_batch_size,
                                       collate_fn=lambda batch: collate_ml(batch, ent_embd),
    ...
    if args.pretrained_dir is not None:
        ...
        model, _ = KemceFTPrediction.from_pretrained(args.pretrained_dir, state_dict)
    else:
        model = KemceFTPrediction(config)
    model.to(device)
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(dataiter, desc="Iteration")):
                ...
                if args.task == 'los':
                    label_task = batch['label_los']
                else:
                    label_task = batch['label_mortality']
